...
- Das Projekt wird/ist ein Vite-Projekt mit React und TypeScript sein.  
  **WICHTIG:** Wenn Du das Vite-Projekt erzeugst, **MUSST** to die existierenden Dateien im Repository belassen, da diese für die Demo benötigt werden! Verändere oder lösche die Dateien **NICHT**, es sei denn, Du wirst explizit dazu aufgefordert!
...
- Wenn Du mit der Englischen Version der Demo arbeitest (d.h. der aktuelle oder Basis-Branch ist `en-US`), erzeuge Pull Requests **IMMER** gegen den `en-US` Branch. Wenn Du mit der Deutschen Version der Demo arbeitest (d.h. der aktuelle oder Basis-Branch ist `main`), erzeuge Pull Requests **IMMER** gegen den `main` Branch.
